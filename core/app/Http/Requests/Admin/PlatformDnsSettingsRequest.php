...
class PlatformDnsSettingsRequest extends FormRequest
{
    ...
    {
        return [
            'platform_domain' => ['required', 'string', 'max:253', 'regex:/^(?=.{1,253}\.?$)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}\.?$/i'],
            'proxy_hostname' => ['required', 'string', 'max:253'],
            'nameservers' => ['required', 'array', 'min:2', 'max:8'],
            'nameservers.*.hostname' => ['required', 'distinct', 'string', 'max:253'],
            'nameservers.*.ipv4' => ['required', 'ipv4'],
            'nameservers.*.ipv6' => ['required', 'ipv6'],
            'soa_primary' => ['required', 'string', 'max:253'],
            'soa_mailbox' => ['required', 'string', 'max:253'],
