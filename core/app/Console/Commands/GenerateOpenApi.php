...
{
    protected $signature = 'cdnf:api:openapi {--check : Fail when the committed contract is stale}';
    ...
    public function handle(): int
    {
        ...
        collect(RouteFacade::getRoutes()->getRoutes())
            ...
            ->each(function (Route $route) use (&$catalog, &$paths): void {
                ...
                foreach (array_diff($route->methods(), ['HEAD', 'OPTIONS']) as $method) {
                    ...
                    $idempotent = in_array('idempotent', $route->gatherMiddleware(), true);
                    ...
                    $catalog[] = [
                        'group' => str_starts_with($path, '/admin/') ? 'Administrator' : ($authenticated ? 'Account' : 'Public'),
        ...
        $jsonPath = base_path('../docs/public/openapi.json');
        $markdownPath = base_path('../docs/reference/api/endpoints.md');
        $markdown = $this->endpointMarkdown($catalog);
        if ($this->option('check')) {
            if (! is_file($jsonPath) || file_get_contents($jsonPath) !== $json) {
                $this->error('docs/public/openapi.json is stale. Run php artisan cdnf:api:openapi.');

                return self::FAILURE;
            }
            if (! is_file($markdownPath) || file_get_contents($markdownPath) !== $markdown) {
                $this->error('docs/reference/api/endpoints.md is stale. Run php artisan cdnf:api:openapi.');

                return self::FAILURE;
            }

            $this->info('OpenAPI contract and endpoint catalog are current.');
        ...
        $this->info('Generated docs/public/openapi.json and docs/reference/api/endpoints.md.');

        return self::SUCCESS;
    }

    private function endpointMarkdown(array $catalog): string
    {
        $lines = [
            '---',
            ...
            '',
            '::: info Read the response semantics',
            'Mutation endpoints may return an operation receipt before runtime activation.',
            "Use the endpoint's status and revision resources to distinguish accepted,",
            'active, failed, and last-valid state.',
            ':::',
            '',
            '<!-- Generated by php artisan cdnf:api:openapi. Do not edit by hand. -->',
            '',
        ...
        foreach (['Public', 'Account', 'Administrator'] as $group) {
            $lines[] = "## {$group} endpoints";
            $lines[] = '';
            $lines[] = '| Method | Path | Bearer auth | Idempotency key | Operation ID |';
            $lines[] = '| --- | --- | --- | --- | --- |';
            foreach (array_filter($catalog, fn (array $endpoint): bool => $endpoint['group'] === $group) as $endpoint) {
                $lines[] = sprintf(
                    '| `%s` | `%s` | %s | %s | `%s` |',
                    $endpoint['method'],
                    $endpoint['path'],
                    $endpoint['authenticated'] ? 'Required' : 'No',
                    $endpoint['idempotent'] ? 'Supported' : 'No',
        ...
        $lines[] = 'Download the machine-readable';
        $lines[] = '[OpenAPI 3.1 contract](https://raw.githubusercontent.com/vaheed/CDNFoundry/main/docs/public/openapi.json).';
        $lines[] = '';
