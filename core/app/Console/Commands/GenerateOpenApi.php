    ...
    public function handle(): int
    {
        ...
        if ($this->option('check')) {
            if (! is_file($jsonPath) || file_get_contents($jsonPath) !== $json) {
                $this->error('docs/public/openapi.json is stale. Run php artisan api:openapi.');

                return self::FAILURE;
            }
            if (! is_file($markdownPath) || file_get_contents($markdownPath) !== $markdown) {
                $this->error('docs/reference/api/endpoints.md is stale. Run php artisan api:openapi.');
