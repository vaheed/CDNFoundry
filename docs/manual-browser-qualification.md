...
The non-browser runtime qualification in `tests/e2e/phase4_runtime.py` covers HTTP routing, the configured origin Host header, verified HTTPS with correct SNI, rejection of invalid origin SNI and blocked destinations, ambiguous-framing rejection, trusted forwarding-header replacement, unknown/disabled/deprovisioned host responses, 2,000 generic hostnames, malformed-state preservation, and hot configuration activation without an OpenResty reload. The Go agent tests cover fresh full-snapshot recovery followed by incremental activation and bounded origin-test execution. These checks do not replace the dual-edge, inbound TLS-SNI, IPv6-client, cell-isolation, mTLS-identity, or rendered-panel checks below.
