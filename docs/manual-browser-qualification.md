...
and strict certificate verification remain **Pending owner run**, so the
release decision remains **Blocked** and the rows below must not be marked
Passed by a coding agent.
