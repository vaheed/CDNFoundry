...
Build the bounded non-root agent container with `docker build -t cdnfoundry/edge-agent:local edge-agent`. In production Compose it has a separate persistent identity/state volume, a read-only root filesystem, 128 MiB memory and 0.25 CPU limits. After the first successful registration, remove `EDGE_BOOTSTRAP_TOKEN` from the environment; restarts load the persisted identity without using it. If the registration response is lost, the agent reuses its persisted pending private key/CSR and the control plane returns the same short-lived certificate for an exact retry; a different CSR fails closed.
