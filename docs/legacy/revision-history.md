...
Managed DNS-01 issuance also reconciles its challenge revision while polling for authoritative acknowledgements. A dropped, failed, or coalesced DNS job is therefore repaired without requiring a user to reissue the certificate; certificate validation cannot begin until every enabled DNS cluster reports that revision or a newer one.
