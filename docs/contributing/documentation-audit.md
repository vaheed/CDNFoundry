...
| Every service address needs an explicit gateway map. | A directly assigned advertised address binds as-is. Behind NAT or a layer-4 load balancer, generated bundles use a complete advertised-to-private `EDGE_GATEWAY_ADDRESS_MAP`; strict sites may require explicit coverage. |
